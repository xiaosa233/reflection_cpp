...
std::string type_name(void*) {
  return "const " + type_name<std::remove_const_t<T>>(0);
}
...
std::string type_name(void*) {
  return type_name<std::remove_pointer_t<T>>(0) + "*";
}

// int* const , const int* const
template <class T, std::enable_if_t<std::is_pointer_v<T> && std::is_const_v<T>, bool> = true>
std::string type_name(void*) {
  return type_name<std::remove_const_t<T>>(0) + "const";
}

template <class T, std::enable_if_t<std::is_lvalue_reference_v<T>, bool> = true>
std::string type_name(void*) {
  return type_name<std::remove_reference_t<T>>(0) + "&";
}
